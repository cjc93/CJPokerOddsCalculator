...
public class Calculation {
    ...
    public void initialise_variables(int[][][] cards, int playersRemainingNo, OutputResult outputResultObj) {
        ...
        for(int player = 1; player <= playersRemainingNo; player++) {
            if(cards[player][0][0] != 0 || cards[player][1][0] != 0) {
    ...
    public String[] convertPlayerCardsToStr(int[][][] cards, int playersRemainingNo) {
        String[] playerCards = new String[playersRemainingNo];

        for (int i = 1; i <= playersRemainingNo; i++) {
            StringBuilder temp = new StringBuilder();
            temp.append(rankToStr.get(cards[i][0][1]));
            temp.append(suitToStr.get(cards[i][0][0]));
            temp.append(rankToStr.get(cards[i][1][1]));
            temp.append(suitToStr.get(cards[i][1][0]));
